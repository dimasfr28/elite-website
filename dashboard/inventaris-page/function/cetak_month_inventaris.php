...
include "session.php";
$post = $_POST['month'];
$select = mysqli_query($conn, "SELECT * FROM tb_peminjaman LEFT JOIN tb_user ON tb_peminjaman.id_user = tb_user.id_user WHERE tb_peminjaman.tanggal_peminjaman LIKE '%$post%'");
$i = 1;
...
<head>
    ...
    <title>Report Peminjaman ELITE | <?= $post; ?></title>
</head>
...
$mpdf->WriteHTML($html);
ob_end_clean();
$mpdf->Output("Report_Peminjaman_ELITE_". $post .".pdf", "D");
?>